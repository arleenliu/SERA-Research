...
with open('vgsales2017.csv') as csvfile:
	...
	A1 = pca1.fit_transform(array1[:][10:13].astype(np.float))
	time1_end = time()

	knn1 = KNN()
	B1 = knn1.fit(A1,array1[:][14])
	knn1.kneighbors_graph(A1, n_neighbors = 10, mode = 'connectivity')
	b_pred = knn1.predict(A1)
	acc1 = m.accuracy_score(array1[:][14], b_pred)
	print ('1st accuracy score is: ' + str(acc1))
	...
	A2 = tsne.fit_transform(array1[:][10:13])
	time2_end = time()

	knn2 = KNN()
	B2 = knn2.fit(A2,array1[:][14])
	knn2.kneighbors_graph(A2, n_neighbors = 10, mode = 'connectivity')
	b2_pred = knn2.predict(A2)
	acc2 = m.accuracy_score(array1[:][14], b2_pred)
	print ('2nd accuracy score is: ' + str(acc2))
	...
	L1 = lsvc1.fit(A1,array1[:][14])
	l_pred = lsvc1.predict(A1)
	acc3 = m.accuracy_score(array1[:][14], l_pred)
	print ('3rd accuracy score is: ' + str(acc3))
	...
	L2 = lsvc2.fit(A2,array1[:][14])
	l2_pred = lsvc2.predict(A2)
	acc4 = m.accuracy_score(array1[:][14], l2_pred)
	print ('4th accuracy score is: ' + str(acc4))
